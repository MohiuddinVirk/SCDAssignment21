...
import UserDetailsDialog from '../components/CustomerDetails';
...
const handleDetailsDialogClose = () => {
  ...
  const performBlockUnblock = async (row, block) => {
    try {
      ...
      const endpoint = !block ? 'disablecustomer' : 'ablecustomer';
  ...
  useEffect(() => {
    const fetchUsersData = async () => {
      try {
        ...
        const response = await fetch(`${ linkurl }/user/getallcutomers`, {
          method: 'GET',
          headers: {
            token: `${token}`,
          },
