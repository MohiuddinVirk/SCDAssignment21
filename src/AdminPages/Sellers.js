...
//       const endpoint = !block ? 'disableseller' : 'ableseller';
...
//         const response = await fetch(`${ linkurl }/user/getallsellers?page=1&pageSize=10`, {
//           method: 'GET',
//           headers: {
//             token: `${token}`,
//           },
...
import { styled } from '@mui/system';
import SideBar from '../components/AdminSideBar';
import {Lock,Visibility,Person,Cancel} from '@mui/icons-material';
import { useMediaQuery } from '@mui/material';
import { useTheme } from '@mui/material/styles';
import { linkurl } from '../link';
...
const UserTable = ({ data, isBlocked, handleBlockUnblock, handleBlockDialogOpen, handleDetailsDialogOpen, selectedUserProjects }) => {
  const columns = [
    { id: 'name', label: 'Name' },
    { id: 'email', label: 'Email' },
    { id: 'password', label: 'Password' },
  ...
  useEffect(() => {
    const fetchUsersData = async () => {
      try {
        ...
        const response = await fetch(`${linkurl}/user/getallsellers`, {
          method: 'GET',
          headers: {
            token: `${token}`,
          },
